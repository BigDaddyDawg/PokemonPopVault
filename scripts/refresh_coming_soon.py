"""Build data/coming-soon.json from Funko shop + news about Pokémon Pops."""
...
UPCOMING_WAVES = [
    {
        "code": "LEGENDARY-BEASTS",
        "name": "Legendary Beasts Jumbo Wave",
        "releaseDate": None,
        "type": "wave",
        "number": 1,
        "blurb": (
...
def parse_funko_preorders(html: str) -> list[dict]:
    """Pull Pokémon Pop tiles that look like pre-order / coming soon."""
    reveals = []
...
def parse_news_markdown(md: str) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        if len(title) < 12 or len(title) > 140:
            continue
        if not re.search(r"pokemon|pokémon|pikachu|eevee|funko", title, re.I):
            # keep general Funko headlines lightly
...
def main() -> None:
    ...
    reveals: list[dict] = []
    try:
        print("Fetching Funko Pokémon listings…")
    ...
    # Fallback: newest high-number pops from baked catalog
    if not reveals:
